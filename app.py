...
def safe_ensure_columns_for_view(df):
    ...
    # เติมคอลัมน์ที่ขาดเพื่อให้ code หน้าเว็บไม่พัง
        ...
        if st.session_state.view_mode == "list":
            ...
            with tab_dash:
                ...
                if not df_display.empty:
                    ...
                    df_display['datetime'] = pd.to_datetime(df_display['Timestamp'], format="%d/%m/%Y %H:%M:%S", errors='coerce')
                    df_display = df_display.dropna(subset=['datetime'])
                    df_display['Hour'] = df_display['datetime'].dt.hour
                    df_display['Day'] = df_display['datetime'].dt.strftime('%A')
                    days_th = {'Monday': 'จันทร์', 'Tuesday': 'อังคาร', 'Wednesday': 'พุธ', 'Thursday': 'พฤหัสบดี', 'Friday': 'ศุกร์', 'Saturday': 'เสาร์', 'Sunday': 'อาทิตย์'}
                    df_display['DayTH'] = df_display['Day'].map(days_th)

                    adv1, adv2 = st.columns(2)
                    with adv1:
                        st.markdown("**🔥 ความสัมพันธ์: สถานที่ vs ประเภทเหตุ**")
                        corr_df = pd.crosstab(df_display['Location'], df_display['Incident_Type'])
                        st.dataframe(corr_df, use_container_width=True, height=300)
                    with adv2:
                        st.markdown("**🕒 ช่วงเวลาเกิดเหตุ (Heatmap Analysis)**")
                        heatmap_df = pd.crosstab(df_display['DayTH'], df_display['Hour'])
...
def main_page():
    ...
    with tab1:
        # [แก้ไขจุดที่ 1] เพิ่ม clear_on_submit=True เพื่อให้ล้างข้อมูลหลังกดส่ง
        with st.form("report_form", clear_on_submit=True):
            ...
            if submitted:
                ...
                if len(det) < 10:
                    ...
                    # --- [เริ่มโค้ดที่แก้ไข: ระบบป้องกันข้อมูลหายขั้นสูงสุด] ---
                    try:
                        # 1. ระบุเป้าหมาย
                        target_sheet = get_target_sheet_name()
                        
                        # 2. อ่านข้อมูลเดิม (พร้อม Retry)
                        df_old = None # 🔴 สำคัญ: ต้องเริ่มด้วย None ห้ามเริ่มด้วย pd.DataFrame()
                        
                        for _ in range(3): # ลองอ่าน 3 ครั้ง
                            try:
                                temp_df = conn.read(worksheet=target_sheet, ttl=0)
                                # ต้องตรวจสอบว่าอ่านได้ และไม่ใช่ None
                        ...
                        # === ⛔ Safety Lock ด่านที่ 1: ตรวจสอบการอ่าน ===
                        if df_old is None:
                            st.error(f"🚨 วิกฤต: ไม่สามารถเชื่อมต่อกับฐานข้อมูล {target_sheet} ได้")
                            st.error("ระบบระงับการบันทึกอัตโนมัติเพื่อป้องกันข้อมูลสูญหาย")
                            st.info("คำแนะนำ: กรุณากดปุ่มส่งข้อมูลใหม่อีกครั้ง")
                            st.stop() # 🛑 สั่งหยุดโปรแกรมทันที

                        # === ⛔ Safety Lock ด่านที่ 2: ตรวจสอบโครงสร้างตาราง ===
                        # ถ้าอ่านมาแล้วไม่มีคอลัมน์ Report_ID แสดงว่าอาจจะอ่านมาผิด หรือ Sheet ว่างเปล่าแบบผิดปกติ
                        if 'Report_ID' not in df_old.columns:
                            # ยกเว้นกรณีเป็น Sheet ใหม่จริงๆ (0 rows) อนุญาตให้ผ่านได้
                            # แต่ถ้ามี row แต่ไม่มี Report_ID คือผิดปกติ
                            if not df_old.empty:
                                st.error("🚨 วิกฤต: โครงสร้างข้อมูลผิดพลาด (ไม่พบคอลัมน์ Report_ID)")
                                st.error("ระบบระงับการบันทึกเพื่อป้องกันการเขียนทับ")
                                st.stop() # 🛑 สั่งหยุดโปรแกรมทันที

                        # 3. เตรียมข้อมูลใหม่
                        # (ใช้ process_image ที่แก้ไขเรื่องขนาดแล้ว)
                        ...
                        new_data = pd.DataFrame([{
                            ...
                            "Audit_Log": f"สร้างรายการเมื่อ: {get_now_th().strftime('%d/%m/%Y %H:%M')}"
                        }])
                        
                        # 4. รวมร่าง (Concatenate)
                        # เติมคอลัมน์ให้ครบก่อนรวม
                        ...
                        # === ⛔ Safety Lock ด่านที่ 3: ตรวจสอบก่อนเขียนลง Sheet ===
                        # จำนวนบรรทัดใหม่ ต้องมากกว่าของเก่าเสมอ (เพราะเราเพิ่มข้อมูล)
                        # ถ้าเท่าเดิมหรือน้อยกว่า แสดงว่าข้อมูลเก่าหายระหว่างทาง
                        if len(combined_df) <= len(df_old):
                             st.error(f"🚨 วิกฤต: ตรวจพบข้อมูลสูญหายผิดปกติ (เก่า {len(df_old)} -> ใหม่ {len(combined_df)})")
                             st.error("❌ ระบบระงับการบันทึกทันที!")
                             st.stop() # 🛑 สั่งหยุดโปรแกรมทันที

                        # 5. บันทึกจริง (Update)
                        conn.update(worksheet=target_sheet, data=combined_df)
                        st.cache_data.clear() # ล้างแคชทันที
                        ...
                        st.warning("⚠️ ข้อมูลยังไม่ถูกบันทึก กรุณาลองใหม่อีกครั้ง")
                    # --- [สิ้นสุดส่วนแก้ไข] ---
        ...
        if st.button("🔎 ค้นหา", use_container_width=True):
            if len(search_code) == 4 and search_code.isdigit():
                try:
                    ...
                    match = df_display[df_display['Report_ID'].str.endswith(search_code)]
