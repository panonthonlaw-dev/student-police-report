        ...
        with st.form("report_form", clear_on_submit=True):
            ...
            if submitted:
                ...
                if len(det) < 10:
                    ...
                    # วางต่อจากบรรทัด rid = ...
                    try:
                        # 1. คำนวณชื่อชีตเป้าหมาย
                        target_sheet = get_target_sheet_name()
                        
                        # 2. อ่านข้อมูลเดิม (เพิ่ม Retry Logic เผื่อเน็ตหลุดตอนเช้า)
                        df_old = pd.DataFrame()
                        
                        # ลองอ่าน 3 ครั้ง ถ้าพลาดให้รอแล้วอ่านใหม่
                        for _ in range(3):
                            try:
                                ...
                                time.sleep(1) # พัก 1 วิแล้วลองใหม่
                        
                        # === [จุดป้องกันข้อมูลหาย 🛑] ===
                        # เช็ค 1: ถ้าอ่านมาแล้ว None หรือไม่มี Columns เลย (แต่ไม่ใช่ชีตใหม่)
                        # เราจะเช็คว่าต้องมีคอลัมน์สำคัญคือ 'Report_ID' 
                        
                        # ถ้าอ่านแล้วได้ DataFrame ว่าง หรือไม่มีคอลัมน์ Report_ID
                        if df_old is None or (not df_old.empty and 'Report_ID' not in df_old.columns):
                            # กรณีนี้คืออ่านไม่เจออะไรเลย หรือโครงสร้างผิด (เสี่ยงมาก)
                            st.error("⚠️ ระบบขัดข้อง: ไม่สามารถดึงฐานข้อมูลเดิมได้ (Read Failed)")
                            st.error("⛔ ระบบระงับการบันทึกเพื่อป้องกันข้อมูลสูญหาย กรุณากดส่งใหม่อีกครั้ง")
                            st.stop() # หยุดการทำงานทันที ห้ามไปบรรทัด conn.update
                        ...
                        # 4. รวมร่าง
                        # ต้องแน่ใจว่า df_old ไม่ใช่ None ก่อน concat
                        if df_old is None: df_old = pd.DataFrame()
                        
                        # เติมคอลัมน์ให้ครบ
                        for c in df_old.columns:
                            if c not in new_data.columns: new_data[c] = ""
                        
                        # รวมข้อมูล (เอาของเก่าตั้ง + ของใหม่)
                        combined_df = pd.concat([df_old, new_data], ignore_index=True)
                        combined_df = combined_df.fillna("")
                        
                        # เช็คครั้งสุดท้ายก่อนบันทึก: จำนวนแถวต้องเพิ่มขึ้น ไม่ใช่น้อยลง
                        if len(combined_df) < len(df_old):
                             st.error("⛔ เกิดข้อผิดพลาดร้ายแรง: ข้อมูลใหม่น้อยกว่าข้อมูลเก่า ระบบระงับการบันทึก")
                             st.stop()

                        # 5. อัปเดตกลับไปที่ชีต
                        ...
                        # แสดง Error จริงๆ ออกมาดูด้วย
                        st.error(f"❌ เกิดข้อผิดพลาด: {str(e)}")
                        st.warning("ข้อมูลยังไม่ถูกบันทึก กรุณาลองใหม่อีกครั้ง")
