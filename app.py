...
def main_page():
    ...
    with tab1:
        ...
        # 1. แสดงปุ่มดึงพิกัด (geo_html ต้องถูกนิยามไว้ก่อนหน้านี้ในโค้ดคุณ)
        components.html(geo_html, height=120)

        # 2. ซ่อนตัวรับค่าพิกัด (แอบไว้ข้างหลัง)
        with st.container():
        ...
        # 3. ใส่ CSS เพื่อสั่งลบแถบสีเทาของช่องด้านบนทิ้งไปเลย
        st.markdown("""
            <style>
        ...
        # 4. เริ่มต้น Form (ตรวจสอบย่อหน้าให้ตรงกับ st.markdown ด้านบน)
        with st.form("report_form", clear_on_submit=True):
            
            # ช่องกรอกข้อมูลข้างในฟอร์ม (ย่อหน้าเข้ามา 1 ระดับ)
